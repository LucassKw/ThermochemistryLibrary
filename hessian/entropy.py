...
class Entropy:
    ...
    def __init__(self, T, mass_kg, principal_moments, frequencies_cm, linear=False):

        self.T = T
        self.m = mass_kg
        self.Ia, self.Ib, self.Ic = np.array(principal_moments)
        self.freqs = np.array(frequencies_cm)
        self.sigma = 1
        self.g_e = 1
        self.linear = linear
